...
class TestRandom(TestCase):
    def test1(self):
        args = pt.TransferArgs(net=config.CIFAR_OVAL_WIDE, domain=Domain.LP, approx=ap.Random(0.02),
                               dataset=Dataset.CIFAR10, eps=2/255,
                               split=Split.RELU_ESIP_SCORE, count=2500, pt_method=IVAN(0.003, 0.003),
                               timeout=SMALL_TIMEOUT)
        pt.proof_transfer(args)
